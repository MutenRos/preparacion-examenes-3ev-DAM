...
function table_meta(): array {
    return [
        'usuarios' => ['title'=>'Usuarios','fields'=>['nombre','apellidos','email','password_hash','perfil_id','activo'],'labels'=>['perfil_id'=>'Perfil ID','password_hash'=>'Hash contraseña']],
        ...
        'recursos' => ['title'=>'Recursos','fields'=>['leccion_id','titulo','descripcion','tipo','url','archivo','orden']],
...
<main class="shell">
<?php }
function layout_end(): void { echo "</main><script src='assets/app.js'></script></body></html>"; }

function login_page(): void {
    $error = '';
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (login_attempt(trim($_POST['email'] ?? ''), $_POST['password'] ?? '')) redirect('index.php');
...
function asignaturas_usuario(array $user): array {
    if (is_manager($user)) {
        return all_rows("SELECT ae.id, a.nombre AS asignatura, ce.nombre AS edicion, c.nombre AS curso, u.nombre || ' ' || COALESCE(u.apellidos,'') AS profesor
            ...
            WHERE ae.activo = 1 ORDER BY c.nombre, a.orden, a.nombre");
    }
    return all_rows("SELECT ae.id, a.nombre AS asignatura, ce.nombre AS edicion, c.nombre AS curso, u.nombre || ' ' || COALESCE(u.apellidos,'') AS profesor
        FROM matriculas m
        ...
        ORDER BY c.nombre, a.orden, a.nombre", [$user['id'], $user['perfil'] === 'profesor' ? 'profesor' : 'alumno']);
}
...
function subject_allowed(array $user, int $ae_id): bool {
    ...
    return (bool)one("SELECT 1 FROM matriculas WHERE usuario_id = ? AND asignatura_edicion_id = ? AND tipo = ? AND activo = 1", [$user['id'], $ae_id, $user['perfil'] === 'profesor' ? 'profesor' : 'alumno']);
}
...
function tree_html(int $asignatura_id, int $ae_id): void {
    ...
    foreach ($unidades as $u) {
        echo '<li><a href="index.php?page=subject&id='.(int)$ae_id.'&type=unidad&item='.(int)$u['id'].'">'.h($u['titulo']).'</a>';
        $subs = all_rows("SELECT * FROM subunidades WHERE unidad_id = ? ORDER BY orden, id", [$u['id']]);
        echo '<ul>';
        foreach ($subs as $s) {
            echo '<li><a href="index.php?page=subject&id='.(int)$ae_id.'&type=subunidad&item='.(int)$s['id'].'">'.h($s['titulo']).'</a>';
            $lessons = all_rows("SELECT * FROM lecciones WHERE subunidad_id = ? ORDER BY orden, id", [$s['id']]);
            echo '<ul>';
            foreach ($lessons as $l) {
                echo '<li><a href="index.php?page=subject&id='.(int)$ae_id.'&type=leccion&item='.(int)$l['id'].'">'.h($l['titulo']).'</a>';
                $sessions = all_rows("SELECT * FROM sesiones WHERE leccion_id = ? ORDER BY fecha, hora_inicio, id", [$l['id']]);
                echo '<ul>';
                foreach ($sessions as $se) {
                    echo '<li><a href="index.php?page=subject&id='.(int)$ae_id.'&type=sesion&item='.(int)$se['id'].'">'.h($se['titulo']).'</a></li>';
                }
                echo '</ul></li>';
            }
            echo '</ul></li>';
...
function selected_content(string $type, int $item): ?array {
    $allowed = ['unidad'=>'unidades','subunidad'=>'subunidades','leccion'=>'lecciones','sesion'=>'sesiones'];
...
function subject_page(array $user): void {
    $ae_id = (int)($_GET['id'] ?? 0);
    if (!$ae_id || !subject_allowed($user, $ae_id)) redirect('index.php');
        ...
        <section class="subject-layout">
            ...
            <article class="reader">
                <div class="reader-head">
                    <div><span><?= h($subject['curso']) ?></span><h1><?= h($selected['titulo'] ?? $subject['asignatura']) ?></h1></div>
                ...
                <div class="prose">
                    <?php if ($selected): ?>
                        ...
                        <?php if ($type === 'sesion'): ?>
                            <p><strong>Fecha:</strong> <?= h($selected['fecha']) ?> · <?= h($selected['hora_inicio']) ?> - <?= h($selected['hora_fin']) ?></p>
                        <?php endif; ?>
                    <?php else: ?>
...
function crud_allowed(array $user, string $table): bool {
    if (is_manager($user)) return isset(table_meta()[$table]);
    return $user['perfil'] === 'profesor' && in_array($table, ['unidades','subunidades','lecciones','sesiones'], true);
}

function crud_page(array $user): void {
    $meta_all = table_meta();
    $table = $_GET['table'] ?? 'unidades';
    if (!crud_allowed($user, $table)) redirect('index.php');
    $meta = $meta_all[$table];
    $fields = $meta['fields'];
    $id = (int)($_GET['edit'] ?? 0);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($table === 'usuarios' && ($data['password_hash'] ?? '') !== '' && !str_starts_with($data['password_hash'], '$2y$')) {
    ...
    <div class="workspace <?= is_manager($user) ? 'with-admin' : '' ?>">
        ...
        <section class="content-main crud">
            ...
            <form method="post" class="crud-form">
                ...
                <?php if ($edit): ?><a class="button secondary" href="index.php?page=crud&table=<?= h($table) ?>">Cancelar edición</a><?php endif; ?>
            </form>
            <div class="table-wrap"><table>
                <thead><tr><th>ID</th><?php foreach ($fields as $f) echo '<th>'.h($f).'</th>'; ?><th></th></tr></thead>
                <tbody>
                <?php foreach ($rows as $r): ?>
                    <tr>
                        <td><?= (int)$r['id'] ?></td>
                        <?php foreach ($fields as $f): ?><td><?= h(mb_strimwidth((string)($r[$f] ?? ''), 0, 70, '…')) ?></td><?php endforeach; ?>
                        <td class="row-actions">
                            ...
                            <form method="post" onsubmit="return confirm('¿Eliminar registro?')">
                                <input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int)$r['id'] ?>"><button class="danger">Eliminar</button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
...
if ($page === 'logout') { logout(); redirect('index.php?page=login'); }
$user = require_login();
if ($page === 'subject') subject_page($user);
elseif ($page === 'crud') crud_page($user);
else dashboard($user);
