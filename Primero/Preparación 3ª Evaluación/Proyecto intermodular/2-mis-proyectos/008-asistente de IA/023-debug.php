...
(() => {
  ...
  AFRAME.registerComponent("auto-avatar-anim", {
    schema: {
      ...
      morphSpeed:{ type: "number", default: 2.0 }
    },

    init: function () {
      this.dbg = document.getElementById("debug3d");

      this.root = null;          // <-- rotate this (whole model)
      this.targetMesh = null;    // <-- morph on this (face mesh)
      this.morphIndex = -1;

      const log = (msg) => {
        if (this.dbg) this.dbg.textContent = msg;
      };
      ...
      this.el.addEventListener("model-loaded", () => {
        ...
        this.root = root; // <-- IMPORTANT: rotate the root
        ...
        // Choose morph mesh (prefer the one that has morph "A")
        let chosen = null;
        ...
        info += "\nIf tooth was static before, this fixes it (root rotation).";
        log(info);

        // Force material update (safe)
        if (chosen.material) {
    ...
    tick: function (timeMs) {
      ...
      // Rotate WHOLE model (body + tooth + everything)
      ...
      // Drive morph ONLY on the mesh that actually has that morph
      if (this.targetMesh &&
          this.morphIndex >= 0 &&
          this.targetMesh.morphTargetInfluences &&
          this.targetMesh.morphTargetInfluences.length > this.morphIndex) {
        const w = (Math.sin(t * this.data.morphSpeed) + 1) * 0.5;
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    if (avatarEntity) {
      avatarEntity.setAttribute("auto-avatar-anim", "morphName: A; rotAmpDeg: 6; rotSpeed: 1; morphSpeed: 2;");
    } else {
