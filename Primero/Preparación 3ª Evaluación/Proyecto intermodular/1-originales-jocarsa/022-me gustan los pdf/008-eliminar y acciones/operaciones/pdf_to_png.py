...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"ok": False, "error": "Uso: pdf_to_png.py manifest.json"}))
        return
    ...
    input_pdf = files[0]
    images_dir = os.path.join(output_dir, "png")
    ...
    try:
        ...
        if not created_files:
            print(json.dumps({"ok": False, "error": "No se generaron imágenes PNG"}))
            return

        zip_path = os.path.join(output_dir, f"{base_name}_png.zip")
