...
<body>
<header class="topbar">
    <a class="brand" href="index.php">jocarsa <span>| formación</span></a>
    <?php if ($user): ?>
        <div class="userbox">
...
function dashboard(array $user): void {
    ...
    <div class="workspace <?= is_manager($user) ? 'with-admin' : '' ?>">
        ...
        <section class="content-main dashboard-main">
...
function tree_link(string $label, int $ae_id, string $type, int $id): string {
    $currentType = $_GET['type'] ?? '';
    $currentItem = (int)($_GET['item'] ?? 0);
    $active = ($currentType === $type && $currentItem === $id) ? ' class="active-tree-item"' : '';

    return '<a'.$active.' href="index.php?page=subject&id='.(int)$ae_id.'&type='.h($type).'&item='.(int)$id.'">'.h($label).'</a>';
}
...
function tree_html(int $asignatura_id, int $ae_id): void {
    ...
    foreach ($unidades as $u) {
        echo '<li>';
        echo tree_link($u['titulo'], $ae_id, 'unidad', (int)$u['id']);
        ...
        foreach ($subs as $s) {
            echo '<li>';
            echo tree_link($s['titulo'], $ae_id, 'subunidad', (int)$s['id']);
            ...
            foreach ($lessons as $l) {
                echo '<li>';
                echo tree_link($l['titulo'], $ae_id, 'leccion', (int)$l['id']);
                ...
                foreach ($sessions as $se) {
                    echo '<li>';
                    echo tree_link($se['titulo'], $ae_id, 'sesion', (int)$se['id']);
                    echo '</li>';
                }
