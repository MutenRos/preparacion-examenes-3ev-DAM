...
def remove_pages(input_pdf: Path, output_pdf: Path, pages_to_remove):
...
def main():
    """
    Usage:
        python3 remove_pages_pdf.py <output_dir> <pages> <file_or_folder> [more...]

    Example:
        python3 remove_pages_pdf.py ./out "1-3,5" file.pdf
    """

    if len(sys.argv) < 4:
        print("Usage:")
        print("  python3 remove_pages_pdf.py <output_dir> <pages> <file_or_folder> [more...]")
        sys.exit(1)
    ...
    for idx, pdf in enumerate(pdfs, start=1):
        try:
            ...
            output_file = output_dir / f"{idx:03d}_{pdf.stem}_removed.pdf"

            remove_pages(pdf, output_file, pages_to_remove)
    ...
    with open(report, "w", encoding="utf-8") as r:
        r.write("REMOVE PAGES PDF REPORT\n")
        r.write("=======================\n\n")
        r.write(f"Pages removed: {pages_str}\n")
