---
# Generación de Informe del Sistema Operativo con Raspberry Pi

Después de aprender sobre la evolución histórica y clasificación de los sistemas operativos, vamos a practicar cómo generar un informe detallado de nuestro sistema operativo utilizando una herramienta en Python. Este ejercicio será especialmente útil si tienes experiencia con Raspberrys o cualquier otro dispositivo similar.

## Enunciado paso a paso

### Instala la herramienta:
1. Descarga el archivo `system_report.py` desde el repositorio de tu clase.
2. Asegúrate de tener Python 3.6+ instalado en tu sistema.

### Ejecuta el programa:
1. Abre una terminal y navega hasta el directorio donde guardaste `system_report.py`.
2. Ejecuta el siguiente comando para generar el informe del sistema:
    ```bash
    python3 system_report.py
    ```

### Analiza los resultados:
* El programa generará un informe en formato legible y JSON.
* Abre `system_report.txt` para ver el informe en formato texto.
* Abre `system_report.json` para examinar la información en formato JSON.

## Restricciones
* No uses librerías externas adicionales.
* Mantén el código dentro de los límites del tema actual.

## Criterios de evaluación
**Introducción y contextualización (25%)**:
- Explica brevemente qué es un informe del sistema operativo y cómo puede ser útil para la gestión y diagnóstico del mismo.
- Menciona cómo el uso de herramientas como `system_report.py` facilita esta tarea.

**Desarrollo técnico correcto y preciso (25%)**:
- Describe los pasos que sigues para ejecutar el programa y analizar los resultados.
- Explica cómo la salida del informe en texto y JSON proporciona información detallada sobre tu sistema operativo.

**Aplicación práctica con ejemplo claro (25%)**:
- Proporciona un ejemplo práctico de cómo generar y leer el informe del sistema utilizando `system_report.py`.
- Menciona cualquier detalle relevante que hayas observado en el informe generado.

**Cierre/Conclusión enlazando con la unidad (25%)**:
- Concluye tu actividad resaltando cómo este ejercicio te ha ayudado a entender mejor la estructura y contenido del informe del sistema.
- Menciona cómo esta experiencia puede ser útil en el futuro para el diagnóstico de problemas o la monitorización del rendimiento del sistema.

---

Para conocer a fondo nuestro sistema operativo, existen herramientas que nos permiten generar informes detallados de todo lo relacionado con el sistema operativo, hardware, software instalado, configuraciones, etc. Para esa misma función, nosotros usaremos un script en `python` de la mano del profe Jose Vicente, que nos permitirá generar un informe detallado de nuestro sistema operativo, tanto en formato texto como en formato JSON.
...
+```
y al ejecutarlo en una consola con python 3.6+ (idealmente con `psutil` instalado) nos generara dos archivos: `system_report.txt` y `system_report.json` con toda la informacion de nuestro sistema operativo, hardware, software, configuraciones, etc.
El resultado en consola sera algo asi:
```
PS C:\Users\freak\Desktop\DAM2527\GIT_DAM_25-27> & C:/Users/freak/AppData/Local/Programs/Python/Python313/python.exe "c:/Users/freak/Desktop/DAM2527/GIT_DAM_25-27/Primero/Sistemas informáticos/002-Instalación de sistemas operativos/001-Evolución histórica y clasificación/101-Ejercicios/002-informacion de mi ordenador.py"
============================================================
INFORME GENERADO
  - Texto legible: C:\Users\freak\Desktop\DAM2527\GIT_DAM_25-27\system_report.txt
  - JSON:         C:\Users\freak\Desktop\DAM2527\GIT_DAM_25-27\system_report.json
============================================================

Resumen (primeras líneas):

=============================INFORME DEL SISTEMA==============================
Generado: 2025-10-12 18:10:46

1) Información Básica
   - Sistema: Windows 11 (AMD64)
   - Plataforma: Windows-11-10.0.22631-SP0
   - Hostname: Jarvis (FQDN: Jarvis)
   - Nodo ID (MAC): 0x2c9811817724
   - Ejecutable Python: C:\Users\freak\AppData\Local\Programs\Python\Python313\python.exe

2) CPU
   - Procesador (platform): AMD64 Family 25 Model 97 Stepping 2, AuthenticAMD
   - Cores (logical): 16, (physical if available): N/D

3) Memoria

4) Discos
   - Raíz total=1999217094656 used=1851326038016 free=147891056640

5) Red
   - IPs detectadas: 192.168.56.1, 192.168.1.115

6) Usuarios & procesos

7) GPU
   - GPUs NVIDIA:
     NVIDIA GeForce RTX 4070 SUPER, 12282 MiB, 560.94

8) Entorno Python
   - Versión: 3.13.7 (tags/v3.13.7:bcee1c3, Aug 14 2025, 14:15:11) [MSC v.1944 64 bit (AMD64)]
   - Módulos (muestra): 002-informacion de mi ordenador, __future__, __hello__, __phello__, _aix_support, _android_support, _apple_support, _asyncio, _bz2, _collections_abc, _colorize, _compat_pickle
...
Si deseas más detalle instala 'psutil' con:
    pip install psutil
```
Despues de esto, podemos usar la informacion obtenida para configurar y monitorizar nuestra red local. Por ejemplo, podemos asignar direcciones IP fijas a ciertos dispositivos en nuestra red, o crear un script que verifique la conectividad con todos los equipos en la red y nos avise si alguno no responde.